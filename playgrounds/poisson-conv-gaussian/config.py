...
work_dir = os.path.join(FILE_ROOT, "playgrounds", "poisson-conv-gaussian")
train_batch_size = 1
micro_batch_size = 1
num_workers = 0  # must be specified carefully!
world_size = 1  # if distributed computing is applied, set it correctly!
noise_model = "poisson-conv-gaussian"
lambda_ = 0.2  # noise model parameter
...
config = dict(
    description="Denoising with score function for PoissonConvGaussian noise "
                "model.",
    running_config=dict(
        model_type="PoissonConvGaussian",
        max_train_step=3,
        max_train_time=1.,
        use_fp16_train=True,
