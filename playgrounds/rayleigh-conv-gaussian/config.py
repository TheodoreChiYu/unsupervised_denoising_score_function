...
work_dir = os.path.join(FILE_ROOT, "playgrounds", "rayleigh-conv-gaussian")
train_batch_size = 1
micro_batch_size = 1
num_workers = 0  # must be specified carefully!
world_size = 1  # if distributed computing is applied, set it correctly!
noise_model = "rayleigh-conv-gaussian"
delta = 0.3  # noise model parameter
conv_3x3_filter = np.array(
    [[0.05, 0.1, 0.05],
     [0.1, 0.4, 0.1],
     [0.05, 0.1, 0.05]]
)  # noise model parameter
sigma = 10.  # noise model parameter
image_size = 128

config = dict(
    description="Denoising with score function for RayleighConvGaussian noise "
                "model.",
    running_config=dict(
        model_type="RayleighConvGaussian",
        max_train_step=3,
        max_train_time=1.,
        use_fp16_train=True,
    ...
    model_config=dict(
        ...
        use_fp16_test=False,  # if cpu is used, better to set it False
        # other model configuration
        additional_sigmas=[0.05, 1e-6, 1e-6],
        milestones=[0, 4950, 5000],
        milestones_intervals=[50, 1],
        delta=delta,
        conv_3x3_filter=conv_3x3_filter,
        sigma=sigma,
        num_iteration=10,
        network_config=dict(
